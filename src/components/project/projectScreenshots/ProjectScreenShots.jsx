...
const ProjectScreenShots = ({ project }) => {
  ...
  const clickRight =()=>(mainImgIdx===length-1)?SetMainImgIdx(0):SetMainImgIdx(length-1)
  const clickLeft =()=>(mainImgIdx===0)?SetMainImgIdx(length-1):SetMainImgIdx(0)
  return (
    <div className=" centerFlex screenShotsWraper">
      <div className="centerFlex sideBtn" onClick={clickLeft}>left</div>
      <div className="centerFlex mainImage" style={{backgroundImage : `url(${project?.screenshots[mainImgIdx]})` }}></div>
